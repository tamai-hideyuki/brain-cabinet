...
> - **3Dライブラリ UI** — ブックマークフォルダを本棚として3D空間に配置、ウォークスルーで探索
> - **ライブラリ検索** — ノートをキーワードで検索、ハイライト表示、クリックでテレポート
> - **ミニマップ** — 上空から全体を俯瞰、クリックでテレポート
...
- [x] **ブックマーク機能** - 階層構造によるノート参照管理
  ...
  - ノード作成/編集/削除
  - 同一ノートを複数フォルダから参照可能
- [x] **UIからのノート作成** - Web UIから直接ノートを作成可能
  - ノート一覧ページに「+ 新規作成」ボタンを追加
  - モーダルでタイトル・内容を入力して即座に作成
...
- [x] **セルフチェック・プロンプト** - 判断記録前の自己確認フロー（GPT側機能）
  - 判断を記録する前に3つの質問を投げかける（確信度・反対意見・感情状態）
  - 過去の類似判断を `decision.search` で検索して提示
  - セルフチェック記録をメモ末尾に自動追加
  - 「すぐ記録して」で即時記録も可能
...
- **ダッシュボード強化** - 5つの新しい分析セクションを追加
  - **📊 クラスタインサイト**: 各クラスタの役割（ドライバー/安定化/ブリッジ/孤立）、凝集度、トレンド、キーワードを可視化
  - **📈 思考トレンド**: 過去7日間のセマンティック変化をバーチャート表示、総変更数・平均変化度・最活発日を表示
  - **🕸️ 影響マップ**: 影響を与える/受けるノートのTop3を2カラムで表示、総エッジ数・平均/最大影響度
  - **💓 ネットワーク健全性**: 円形ゲージで健全度と孤立率を表示、良好接続/孤立/未接続ノート数の統計
  - **🗓️ 活動カレンダー**: GitHubスタイルの30日ヒートマップ、総活動・活動日・日平均を表示
- **活動カレンダー改善**:
  - ノート作成・更新を即時反映（noteHistoryに加えnotes.createdAt/updatedAtも集計）
  - 月またぎ対応（週の途中で月が変わってもラベルを正しく表示）
  - 全日付を埋める処理（データがない日も表示）
- **iPhoneレスポンシブ対応**: 全新セクションでiPhone SE〜iPhone Plus/Maxまで対応
...
- **パフォーマンスメトリクス・ダッシュボード**: クライアントサイド完結型のモニタリング機能
  - **`_bcMeta` レスポンスフィールド**: APIレスポンスにサーバー処理時間を含むメタデータを追加
    - `serverLatency`: サーバーでの実行時間（ms）
    - `cached`: キャッシュヒットしたか（将来拡張用）
    - `action`: 実行したコマンド名
  - **IndexedDB メトリクスストア**: 直近1000件のリクエストをローカルに保存
    - サーバー処理時間、ネットワーク時間、ペイロードサイズを記録
    - アクション別の集計機能
  - **ステータスバー**: 画面下部にリアルタイムでレイテンシを表示
    - 色分け表示: 緑（<100ms）、黄（100-300ms）、赤（>300ms）
    - 展開して直近5件のリクエスト詳細を確認可能
  - **システムページ拡張**: `/ui/system#metrics` で詳細ダッシュボードを表示
    - 総リクエスト数、平均レイテンシ、ペイロードサイズ
    - アクション別統計、直近リクエスト一覧
    - メトリクスクリア機能
  - **設計思想**: サーバーへの追加負荷ゼロ、クライアントサイド完結型
...
### v5.12.0
- **キャッシュレイヤー**: 重い分析エンドポイントのTTLキャッシュ
...
### v5.10.0
- **ドリフト方向追跡**: 思考の進化パターンを可視化
  - driftVector = newEmbedding - oldEmbedding
  - クラスター間フロー分析
  - trajectory: expansion/contraction/pivot/lateral/stable

### v5.9.0
- **マルチタイムスケール分析**: 複数時間軸でのトレンド追跡
  - 週次/月次/四半期トレンドの並列分析
  - 季節パターン検出
  - フェーズ遷移検出（dormant/emerging/expansion/consolidation/declining）

### v5.8.0
- **クラスター品質メトリクス**: クラスタリングの品質を定量評価
  - シルエットスコア、Davies-Bouldin Index
  - クラスタ凝集度・分離度の計算

### v5.7.0
- **時間減衰機能**: 古いノートの影響を時間経過で減衰
  - 指数減衰モデル（半減期ベース）
  - 検索スコア・クラスタリングへの適用

### v5.6.0
- **インフルエンスグラフ強化**: ノート間の影響関係を詳細に分析
  - 双方向影響度の計算
  - 影響伝播パスの可視化
...
- **UIからのノート作成**: Web UIから直接ノートを作成可能
  - ノート一覧ページに「+ 新規作成」ボタンを追加
  - `CreateNoteModal` コンポーネントでタイトル・内容を入力
  - **作成のみUIから可能**（編集・更新・削除はGPT経由で実行）
  - GPTからは引き続きCRUD全操作が可能（従来通り）

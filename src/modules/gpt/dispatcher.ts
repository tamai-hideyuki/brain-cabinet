...
import * as gptService from "./services";
import { getGptContext } from "./services/context/gptContext";
import { searchNotes, searchNotesSemantic, searchNotesHybrid, type SearchResult } from "../search";
...
export const gptDispatcher = {
  async search(payload: unknown) {
    ...
    switch (mode) {
      case "semantic":
        results = await searchNotesSemantic(p.query, options);
        break;
      case "keyword":
        results = await searchNotes(p.query, options);
        break;
      case "hybrid":
      default:
        results = await searchNotesHybrid(p.query, options);
        break;
    }

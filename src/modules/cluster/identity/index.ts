...
import * as clusterRepo from "../repository";
import type {
  ClusterIdentity,
  RepresentativeNote,
  ClusterDriftSummary,
...
import { round4, bufferToFloat32Array, cosineSimilarity } from "../../../shared/utils/math";
...
🚨【必須ステップ - 人格化出力後の評価保存】
人格化出力を生成したら、ユーザーへの応答を返す前に必ず system.evaluateVoice を呼び出して評価を保存してください。
これを省略すると、観測者ルール遵守の記録が残らず、声の混在を検出できません。

手順:
1. 人格化出力を JSON 形式で生成
2. 即座に system.evaluateVoice を呼び出し（ユーザー確認不要）
3. 評価結果と共にユーザーへ応答
...
呼び出しフォーマット:
{
  ...
  "payload": {
    "output": { /* 生成した人格化出力（上記フォーマット準拠）*/ },
    "promptVersion": "v7.1.0-observer"
  }
}

⚠️ 評価呼び出しをスキップしないでください。これは品質保証のための必須プロセスです。`;
